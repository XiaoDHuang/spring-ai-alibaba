...
| E11 | 新接口 `/api/prompt/version/diff` 不在任何鉴权拦截器范围内（当前 `/api/prompt/*` 未被 `TokenAuthInterceptor` 或 `ApiKeyAuthInterceptor` 覆盖） | `InterceptorConfig.java` 追加 `.addPathPatterns("/api/prompt/**")` 到 `TokenAuthInterceptor`，确保 diff 接口需登录 | 影响分析中发现的鉴权缺口；代码推断 |
...
| `/api/**` 路径默认不被鉴权拦截器覆盖（`TokenAuthInterceptor` 仅覆盖 `/console/v1/**`；`ApiKeyAuthInterceptor` 仅覆盖 `/api/v1/**`），新增 `/api/**` 接口需在 `InterceptorConfig.java` 显式注册 | CLAUDE.md — 本次新增 | diff 接口需在 `InterceptorConfig` 中追加 `/api/prompt/**` 到 Token 拦截器（见 E11） |
