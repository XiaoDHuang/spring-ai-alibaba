...
```bash
grep -n 'CREATE TABLE\|^\s*`\|^\s*[a-z]' spring-ai-alibaba-admin/docker/middleware/init/mysql/admin-schema.sql
grep -n 'CREATE TABLE\|^\s*`\|^\s*[a-z]' spring-ai-alibaba-admin/docker/middleware/init/mysql/agentscope-schema.sql
```
